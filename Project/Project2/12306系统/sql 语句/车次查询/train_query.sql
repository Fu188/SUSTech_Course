create or replace function train_query(trainNo varchar, dptdate date)
    returns table
            (
                train_no        varchar,
                train_id        int,
...
BEGIN
    ...
    for result in (with train_number as (
        ...
        where t.train_no = trainNo
          and t.start_date = dptdate
          and ts1.station_no = 1
          and ts2.station_no = maxStationNo)
                   ...
                   ) sub2 on sub2.train_id = sub.train_id
                   order by (sub2.station_no))
        LOOP
