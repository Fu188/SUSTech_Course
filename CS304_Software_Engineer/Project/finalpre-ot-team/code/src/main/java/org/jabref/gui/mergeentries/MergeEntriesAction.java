...
public class MergeEntriesAction extends SimpleCommand {
    ...
    /**
     * merges two entries based on the following strategy:
     * 1. if two entry's citation key are empty, the above one will be selected;
     * 2. if only one entry has citation key, then it will be selected;
     * 3. if two entries both have citation key, the one who has smaller dictionary order will be selected.
     */
    @Override
    public void execute() {
